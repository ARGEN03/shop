    ...
    def post(self, request):
        ...
        if user:
            try:
                ...
                # Отправка письма с подтверждением асинхронно с использованием Celery
                send_confirmation_email_task.delay(user.email, user.activation_code)
            except Exception as e:
                return Response(
                    {
                        'message': 'Ошибка при отправке электронного письма: {}'.format(str(e)),
...
class LogoutView(APIView):
    ...
    def post(self, request):
        try:
            ...
            token.blacklist()
            return Response({'message':'Успешно разслогинились!'}, 200)
        except Exception as e:
